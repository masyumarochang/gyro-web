...
petalImg.src = 'img/kukuchikuntofu.png';
...
function generatePetal(fromTop = true) {
    ...
    petal.swayPhase = rand(0,2 * Math.PI);
    petal.swayAmp = rand(4,16)
...
function nomalizeScreenAxes(accelX,accelY) {
    const screenAngle = ((getScreenAngle() % 360) + 360) % 360;
...
function onMotion(e) {
    ...
    const {virtualAccelX, virtualAccelY, screenAngle} = nomalizeScreenAxes(
        accelX,
        accelY
    );
